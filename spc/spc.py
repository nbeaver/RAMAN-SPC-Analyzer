...
( Slightly modified by John Ferrier to allow for easier data analysis and reading of code )
"""
...
class File:
    ...
    ############ Custom added code by John Ferrier for the Rennishaw RAMAN output ##################
    def data_list(self):
        ...
        if self.fnsub == 1:
            ...
            for x1, y1 in zip(x, y):
                ...
                dat_x.append( float(x1) )
                dat_y.append( float(y1) )
            ...
            if not self.dat_fmt.endswith('-xy'):
                ...
                for i in range(len(self.x)):
                    ...
                    dat_x.append( float(self.x[i]) )

                    for s in self.sub:

                        dat_y.append( float(s.y[i]) )
            else:
                #txyxy format, return one long xy file with subfiles
                #Separated by blank lines
                for i in self.sub:
        ...
        #Append separated values with common indices
        dat.append(dat_x)
        dat.append(dat_y)
